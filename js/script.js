      ...
      border: 1px solid #EB5757;border-radius: 16px;" id="modal-id-1">
        ...
        <div class="md:flex md:flex-row gap-2  my-4 ">
            <p class="p-2 mb-2  text-[#03A30A] font-bold text-sm text-center" style="background: #FFFFFF;
              border-radius: 16px;">
                      <span>${item?.pricing ? item.pricing[0]?.price : 'No pricing'}</span><br> 
                      <span>${item?.pricing ? item.pricing[0]?.plan : 'No Plan'}</span></p>
                    <p class=" mb-2 p-2  text-[#F28927] font-bold text-sm text-center" style="background: #FFFFFF;
                    border-radius: 16px;">
                      <span>${item?.pricing ? item.pricing[1].price : 'No pricing'}</span><br>
                      <span>${item?.pricing ? item.pricing[1].plan : 'No plan'}</span></p>
                    <p class="p-2 text-[#EB5757] font-bold text-sm text-center" style="background: #FFFFFF;border-radius: 16px;">
                      <span>${item?.pricing ? item.pricing[2].price : 'No pricing'}</span><br>
                      <span>${item?.pricing ? item.pricing[2].plan : 'No plan'}</span></p>
                    </div>
                    <div class="md:grid md:grid-cols-2 w-full  gap-4">
                      <div class="">
                        ...
                        <ul class="list-disc mt-3  pl-8">
                          ...
                          ${item?.features ? item?.features[1]?.feature_name :'No Feature'}</li>
                          <li class="font-normal text-sm text-[#585858]">
                          ${item?.features? item?.features[2]?.feature_name : 'No feature'}</li>
                          <li class="font-normal text-sm text-[#585858]">
                          ${item?.features? item?.features[3]?.feature_name : 'No feature'}</li>
                        </ul>
                      </div>
                      <div class=" ">
                        ...
                        <ul class="list-disc mt-3 pl-8">
                          <li class="font-normal text-sm text-[#585858]">
                          ${item?.integrations ? item.integrations[0] : 'No Integrations'}</li>
                          <li class="font-normal text-sm text-[#585858]">
                          ${item?.integrations ? item.integrations[1] : 'No Integrations'}</li>
                          <li class="font-normal text-sm text-[#585858]">
                          ${item?.integrations ? item.integrations[2] : 'No Integration'}</li>
                        </ul>
                      </div>
                    </div>
                  </div>
          ...
          <div class="">
            <div style="position: relative;">
             ...
             <div style="position: absolute; top: 12%; right: 2%; ">
                ...
                ${item?.accuracy? item?.accuracy?.score : 'No score'}% accuracy</span>
             </div>
           </div>
        </div>
        <h3 class="mt-4 mb-4 text-[#111111] text-center text-base font-semibold">
        ${item?.input_output_examples? item.input_output_examples[0].input: 'No input'}</h3>
        <p class="text-center  text-xs text-[#585858]  font-normal">
        ${item?.input_output_examples? item.input_output_examples[0].output : 'No output'}</p>
   </div>
